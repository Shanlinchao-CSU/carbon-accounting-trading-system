<template>
  <el-table
    ...
    <el-table-column label="排放限额" width="140">
      <template #default="scope">
        <span v-if="scope.row.account_id !== chosen_id">200</span>
        <el-input
            size="small"
    ...
    <el-table-column prop="t_coin" label="炭币余额" width="140"/>
      ...
      <template #default="scope">
        <div style="display: flex;justify-content: space-around">
          <el-button
              link
              type="success"
              @click="chosen_id = scope.row.account_id"
...
function Searching() {
  if (!search_input.value || !search_input.value.trim()){
    ...
    all_data.value.forEach(item => {
      if (item.account_id.includes(search_input.value)
